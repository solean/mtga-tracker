...
import { buildGraphPoints, type Ladder } from "../src/lib/rankProgress";
...
function resultsFor(history: RankHistoryPoint[], ladder: Ladder): string[] {
  return buildGraphPoints(history, ladder)?.points.map((entry) => `${entry.eventName}:${entry.result}`) ?? [];
}
